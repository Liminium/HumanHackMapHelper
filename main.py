...
class MapHelper(QMainWindow, UiFunctionality, EasterEggsFunctionality):
    ...
    def __init__(self):
        ...
        self.org_longitude, self.org_latitude, self.org_dots, self.org_scale = None, None, None, None
        self.user_json_file, self.user_image_file, self.current_bytes_image, self.json_data, self.org_json_data = None, None, None, None, None
        self.found_organisations_list = list()
        self.image_url_pattern = re.compile(r"background-image:\s?(url\(.+\))")
        self.session_id = f"{dt.now().strftime('%f%S%M%H%d%m%Y')}"
        self.image.wheelEvent = self.image_wheel_event
        ...
        self.empty_radius_btn.clicked.connect(lambda: self.radius_lineedit.setText(""))
        self.empty_results.clicked.connect(lambda: self.results_lineedit.setText(""))
        self.empty_organisation_btn.clicked.connect(lambda: self.oganisation_lineedit.setText(''))
        ...
        self.address_lineedit.textChanged.connect(lambda: self.check_correct_input_address(self.address_error_label, self.address_icon_error))
        self.oganisation_lineedit.textChanged.connect(lambda: self.check_correct_input_address(self.org_error_label, self.org_icon_error))
        self.place_lineedit.textChanged.connect(lambda: self.check_correct_input_address(self.place_error_label, self.place_icon_error))
        self.scale_lineedit.textChanged.connect(self.check_correct_input_scale)
        self.radius_lineedit.textChanged.connect(self.check_correct_input_radius)
        self.results_lineedit.textChanged.connect(self.check_correct_input_results)
        ...
        # Search button
        self.search_organisation_button.clicked.connect(self.organisation_search)
        ...
        self.gear_button.clicked.connect(lambda: self.stacked.setCurrentIndex(5))
        self.info_button.clicked.connect(lambda: self.stacked.setCurrentIndex(2))
        self.org_button.clicked.connect(lambda: self.stacked.setCurrentIndex(1))
        ...
        self.logFile_lineedit.setText(get_json_data(USER_JSON_DATA).get("log_file", ""))
        ...
        self.btn_telegram.clicked.connect(lambda: wb.open('https://t.me/DidenkoD'))
        ...
        self.auto_geo_location.clicked.connect(lambda: self.oganisation_lineedit.setText(self.get_address(self.device_location)))

        self.org_info_btn.clicked.connect(self.show_org_list)
    ...
    @staticmethod
    def make_image(image: str, data: bytes) -> None:
    ...
    def device_location(self) -> str:
        ...
        try:
            dictionary = geocoder.ip("me").geojson["features"][0]["properties"]
        except IndexError:
        ...
        return ','.join(map(str, (dictionary['lng'], dictionary['lat'])))

    def change_image_property(self) -> None:
        """Changing image properties"""
        if self.latitude is not None:
            self.make_image(FILE_IMAGE_NAME, self.get_image(self.longitude, self.latitude, self.scale))
    ...
    def check_correct_input_address(self, error_label: QLabel, error_icon: QPushButton) -> bool:
        """Checks the validness of the specified address"""
        address_text = self.sender().text().strip()

        if any(ec in string.ascii_letters for ec in address_text) and any(rc in RUSSIAN_LETTERS for rc in address_text):
            error_label.setText("Address contains letters of different languages")
            return False

        if address_text:
            if not re.fullmatch(r"[0-9А-Я-A-ZЁ/№# .,;_-]+", address_text, flags=re.I):
                error_label.setText("Address may contain incorrect symbols")
                return False

        if len(address_text) > 3:
            if re.fullmatch(r"[^A-ZА-Я]+", address_text, flags=re.IGNORECASE):
                error_label.setText("Address may be incorrect")
                return False

        error_label.setStyleSheet(WARNING_TITLE_STYLE)
        error_label.setText('')
        error_icon.setStyleSheet("image: none")
        self.sender().setStyleSheet(ORDINARY_LINEEDIT_STYLE)
        return True
    ...
    def check_correct_input_radius(self):
        """Checks validness of specified radius"""
        text = self.radius_lineedit.text()

        if text:
            if not text.isdigit():
                self.radius_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                self.radius_error_label.setText("Invalid value")
                self.radius_icon_error.setStyleSheet(WARNING_ICON)
                return False

            if int(text) > 100:
                self.radius_icon_error.setStyleSheet(WARNING_ICON)
                self.radius_error_label.setText("Too large value")
                self.radius_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                return False

        self.radius_lineedit.setStyleSheet(ORDINARY_LINEEDIT_STYLE)
        self.radius_error_label.setText("")
        self.radius_icon_error.setStyleSheet("image: none")
        return True

    def check_correct_input_results(self):
        text = self.results_lineedit.text()

        if text:
            if not text.isdigit():
                self.results_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                self.results_error_label.setText("Invalid value")
                self.results_icon_error.setStyleSheet(WARNING_ICON)
                return False

            if int(text) > 500:
                self.results_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                self.results_error_label.setText("Too large value")
                self.results_icon_error.setStyleSheet(WARNING_ICON)
                return False

        self.results_lineedit.setStyleSheet(ORDINARY_LINEEDIT_STYLE)
        self.results_error_label.setText('')
        self.results_icon_error.setStyleSheet("image: none")
        return True
    ...
    def organisation_search(self) -> None:
        """Searches organisations nearby"""
        _return = False

        if not self.oganisation_lineedit.text().split():
            self.oganisation_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
            self.org_error_label.setStyleSheet(ERROR_TITLE_STYLE)
            self.org_error_label.setText("Address is required")
            self.org_icon_error.setStyleSheet(WARNING_ICON)
            _return = True

        if not self.place_lineedit.text().split():
            self.place_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
            self.place_error_label.setStyleSheet(ERROR_TITLE_STYLE)
            self.place_error_label.setText("Organisation is required")
            self.place_icon_error.setStyleSheet(WARNING_ICON)
            _return = True

        if not self.check_correct_input_radius() or not self.radius_lineedit.text():
            if self.check_correct_input_radius():
                self.radius_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                self.radius_error_label.setText("Radius is required")
                self.radius_icon_error.setStyleSheet(WARNING_ICON)
            _return = True

        if not self.check_correct_input_results() or not self.results_lineedit.text():
            if self.check_correct_input_results():
                self.results_lineedit.setStyleSheet(ERROR_LINEEDIT_STYLE)
                self.results_error_label.setText("Results number s required")
                self.results_icon_error.setStyleSheet(WARNING_ICON)
            _return = True

        if _return:  # If some checks didn't pass then return
            return

        self.org_error_label.setStyleSheet(WARNING_TITLE_STYLE)
        self.place_error_label.setStyleSheet(WARNING_TITLE_STYLE)

        temp_data = self.request_get('http://geocode-maps.yandex.ru/1.x',
                                     params={'format': 'json',
                                             'apikey': GEOCODE_APIKEY,
                                             'geocode': self.oganisation_lineedit.text()},
                                     timeout=.5)
        ...
        if json_data["response"]["GeoObjectCollection"]["metaDataProperty"]["GeocoderResponseMetaData"]["found"] == '0':
            self.org_icon_error.setStyleSheet(WARNING_ICON)
            self.org_error_label.setStyleSheet(ERROR_TITLE_STYLE)
            self.org_error_label.setText("No results for current address")
            self.log_record("No results for current organisation address.", "warning")  # Logging
            return

        self.found_organisations_list.clear()

        ll_coordinates = json_data["response"]["GeoObjectCollection"]["featureMember"][0]["GeoObject"]["Point"]["pos"].split()

        degrees = int(self.radius_lineedit.text()) * 180 / (6000 * math.pi)

        temp_data = self.request_get('https://search-maps.yandex.ru/v1/',
                                     params={"apikey": ORGANISATIONS_APIKEY,
                                             'text': self.place_lineedit.text(),
                                             'lang': self.system_languages[self.org_lang_combobox.currentText()],
                                             'll': ','.join(ll_coordinates),
                                             'spn': f"{degrees},{degrees}",
                                             'rspn': 1,
                                             'results': self.results_lineedit.text()})
        ...
        if not json_data['features']:
            self.place_icon_error.setStyleSheet(WARNING_ICON)
            self.place_error_label.setStyleSheet(ERROR_TITLE_STYLE)
            self.place_error_label.setText("No results for current organisation")
            self.log_record("No results for current organisation address.", "warning")  # Logging
            return

        self.address_button_2.setText(f"Point founds: {json_data['features'].__len__()}")
        self.coordinates_button_2.setText(f"Current coordinates: {', '.join(ll_coordinates)}")
        self.postal_code_btn_2.setText(F"Places nearby: {json_data['properties']['ResponseMetaData']['SearchResponse']['found']}")
        with open('asd.json', mode='w', encoding="UTf8") as f:
            json.dump(temp_data.json(), f, ensure_ascii=False, indent=8)

        for org in json_data['features'][:int(self.results_lineedit.text())]:
            props = org['properties']
            org_id = props['CompanyMetaData']['id']
            name = props['name']
            address = props['description']
            coordinates = ', '.join(map(str, org['geometry']['coordinates']))
            categories = ', '.join(i['class'] for i in props['CompanyMetaData']['Categories']).capitalize()
            hours_temp_data = props['CompanyMetaData']['Hours']["Availabilities"]
            if 'TwentyFourHours' in hours_temp_data[0] and 'Everyday' in hours_temp_data[0]:
                hours = '24-hours, everyday'
            elif 'Everyday' in hours_temp_data[0]:
                time = hours_temp_data[0]["Intervals"][0]
                hours = f'{time["from"][:5]}-{time["to"][:5]}, everyday'
            else:
                text: str = props['CompanyMetaData']['Hours']['text']
                text = text.replace('пн', 'Mon')
                text = text.replace('вт', 'Tue')
                text = text.replace('ср', 'Wed')
                text = text.replace('чт', 'Thurs')
                text = text.replace('пт', 'Fri')
                text = text.replace('сб', 'Sat')
        ...
        longitudes = [float(i.coordinates.split(',')[1].strip()) for i in self.found_organisations_list]
        latitudes = [float(i.coordinates.split(',')[0].strip()) for i in self.found_organisations_list]

        delta_longitude = max(longitudes) - min(longitudes)
        delta_latitude = max(latitudes) - min(latitudes)
        ...
        params = {'ll': ','.join(ll_coordinates),
                  'spn': ','.join(map(str, (round(delta_longitude, 6), round(delta_latitude, 6)))),
                  'l': self.image_types[self.image_combobox.currentText()],
                  "size": f'{self.org_image.width()},{self.org_image.height()}',
                  "lang": self.system_languages[self.org_lang_combobox.currentText()],
                  'pt': points}

        response = self.request_get("http://static-maps.yandex.ru/1.x", params=params)
        ...
        self.make_image(FILE_ORGANISATION_IMAGE_NAME, response.content)
        self.org_image.setPixmap(QPixmap(FILE_ORGANISATION_IMAGE_NAME))
#        pprint.pprint(self.found_organisations_list)

    def image_wheel_event(self, event=None) -> None:
        # If search has not been started yet
        if self.longitude is None:
            return

        minimum_delta, delta = 1000, event.angleDelta().y() // -120

        for value in MAP_SCALES:
            if abs(self.org_scale - value) < minimum_delta:
                index = MAP_SCALES.index(value)
                minimum_delta = abs(value - self.org_scale)
        ...
        # Запись в файл
        self.make_image(FILE_ORGANISATION_IMAGE_NAME, self.get_image(self.longitude, self.latitude, self.scale))
        self.image.setPixmap(QPixmap(FILE_IMAGE_NAME))
    ...
    def get_address(self, coordinates: str) -> str:
        """Get address from coordinates"""
        request = self.request_get('http://geocode-maps.yandex.ru/1.x',
                                params={'apikey': GEOCODE_APIKEY,
                                        'format': 'json',
                                        'geocode': coordinates}).json()

        toponym = request["response"]["GeoObjectCollection"]["featureMember"][0]
        return toponym["GeoObject"]["metaDataProperty"]["GeocoderMetaData"]["text"]
    ...
    def show_org_list(self):
        print(self.found_organisations_list[0]._fields)
        OrganisationList(self.found_organisations_list).exec()
        print('asdasd')
    ...
    def apply_log_settings(self) -> None:
        ...
        if not (self.logFile_lineedit.text() and self.rows_num_lineedit.text()):
    ...
    def show(self, is_being_opened=None):
...
if __name__ == '__main__':
    ...
    app = SplashScreen(MapHelper)  # todo
    app.show()
